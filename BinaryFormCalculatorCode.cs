...
namespace WindowsFormsApp5
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            //MessageBox.Show(Convert.ToString(GCD(GCD(5,-4), GCD(-4,7))));
        }
        ...
        //https://math.berkeley.edu/~mcivor/math115su12/HW/HW5soln.pdf problems 3,4, and 5


        public void Calculate()
        {
            int discriminant = Convert.ToInt32(textBox1.Text); //value of given discriminant is retrieved
            int classNumber = 0;  //This is a counter variable that will be incrememnted each time a reduced form is found
            ...
            for (int i = 1; i <= upperBound; i++)    // i represents all possible values of a, while j represents all possible values of b for each value of a. This loop structure is assumed by following the solutions to problems 3,4, and 5 from hyperlinked document above
            {
                ...
                for (int j = -i; j <= i; j++)
                {
                    ...
                    //if (discriminant % 2 != j % 2)   // D and b must have the same parity. Since j represents all possible b values, the values that do not share parity with the discriminant are discarded with the continue statement
                    //{
                      //  continue;
                    //}
                   ...
                   // MessageBox.Show("(" + Convert.ToString(i) + "," + Convert.ToString(j) + "," + Convert.ToString(c));
                    if (c - Math.Floor(c) != 0) 
